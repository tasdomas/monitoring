...
type DialCollectorSuite struct {
	...
	collector *monitoring.DialCollector
}
...
func (s *DialCollectorSuite) SetUpTest(c *gc.C) {
	...
	s.collector = monitoring.NewDialCollector("test")
}
...
func (s *DialCollectorSuite) TestDescribe(c *gc.C) {
	...
	c.Assert(descs[0].String(), gc.Matches, `.*fqName: "test_mongo_dials_total".*`)
	c.Assert(descs[1].String(), gc.Matches, `.*fqName: "test_mongo_dial_duration_seconds".*`)
}
