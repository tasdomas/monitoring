...
type TxnCollectorSuite struct {
	...
	collector *monitoring.TxnCollector
}
...
func (s *TxnCollectorSuite) SetUpTest(c *gc.C) {
	...
	s.collector = monitoring.NewTxnCollector("test")
}
...
func (s *TxnCollectorSuite) TestDescribe(c *gc.C) {
	...
	c.Assert(descs[0].String(), gc.Matches, `.*fqName: "test_mgo_txn_ops_total".*`)
}

func (s *TxnCollectorSuite) TestCollect(c *gc.C) {
	s.collector.AfterRunTransaction("dbname", []txn.Op{{
		C:      "update-coll",
		Update: bson.D{},
	}, {
		C:      "insert-coll",
